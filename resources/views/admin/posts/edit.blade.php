...
<div class="max-w-3xl space-y-6">
    ...
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-5">
        ...
        <div>
            ...
            <textarea id="content-editor" name="content" required class="w-full px-4 py-3 bg-slate-950 border border-slate-700 focus:border-blue-500 rounded-xl text-white text-sm outline-none transition">{{ old('content', $post->content) }}</textarea>
        </div>
        ...
        <div><label class="block text-xs font-semibold text-slate-400 mb-1.5">Thumbnail Baru — max 10MB, otomatis dikompres (opsional)</label><input type="file" name="thumbnail" accept=".jpg,.jpeg,.png,.webp" class="w-full text-slate-400 text-sm">@if($post->thumbnail)<p class="text-xs text-slate-500 mt-1">Thumbnail saat ini: {{ basename($post->thumbnail) }}</p>@endif</div>
